...
Z f(int N, vector<tuple<int, int, long long>> constr, bool quant = false) {
    int M = constr.size();
    vector<int> wt = {1};
    for (auto [l, r, _]: constr) {
        wt.push_back(wt.back() * (r - l) * 2);
    }
    vector<int> H(N);
    for (int i = 0; i < N; i++)
        for (int j = 0; j < M; j++) {
    ...
    for (auto [l, r, _]: constr) {
        int z = tw.size();
        for (int t = 1; t < (r - l); t++) {
            for (int i = 0; i < z; i++) tw.push_back(tw[i] >= 0 ? z * 2 * t + tw[i] : -1);
        }
        for (int t = 0; t < (r - l); t++) {
            for (int i = 0; i < z; i++) tw.push_back(-1);
        }
    }
    vector<Z> dp(wt.back());
    dp[0] = 1;
    for (int t = 0; t < 30; t++) {
        int off = 0;
        for (int j = 0; j < M; j++)
            if ((get<2>(constr[j])) & 1ll << t) off += wt[j];
        for (int h: H)
            for (int k = wt.back() - 1; k >= h; k--) dp[k] += dp[k - h];
        vector<Z> nx(wt.back());
        for (int i = 0; i < wt.back(); i++)
            if (tw[i] >= 0) nx[i] = dp[tw[i] + off];
        swap(dp, nx);
        if (!quant)
            for (auto &q: dp)
                if (q.val()) q = 1;
...
void solve() {
    int N, M;
    cin >> N >> M;
    vector D(N + 1, vector<long long>(N + 1, INF));
    auto ch = [&](int u, int v, long long s) -> bool {
        if (D[u][v] != INF && D[u][v] != s) return false;
        D[u][v] = s;
        return true;
    };
    for (int i = 0; i < M; i++) {
    ...
    vector<tuple<int, int, long long>> constr;
    for (int l = 0; l < N; l++) {
        int r = l + 1;
        while (r <= N && D[l][r] == INF) r++;
        if (r <= N) constr.push_back({l, r, D[l][r]});
    }
    ...
    vector<int> ha(N);
    for (auto [l, r, w]: constr) {
        for (int i = l; i < r; i++) ha[i] = 1;
    }
    for (int i = 0; i < N; i++)
        if (ha[i] == 0) {
            if (f(N, constr, false).val())
                cout << "Infinity\n";
            else
                cout << "0\n";
            return;
        }
    cout << f(N, constr, true).val() << "\n";
}
int main() {
