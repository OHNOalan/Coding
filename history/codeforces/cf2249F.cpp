...
public:
    MaximumWeightedMatching(int N, const vector<InputEdge> &in) :
        N(N), B((N - 1) / 2), S(N + B + 1), ofs(N + 2), edges(in.size() * 2), heap2(S), heap2s(S, S),
        heap3(edges.size()), heap4(S) {
    ...
    vector<int> get_matching() { return mate; }
...
void solve() {
    int n, m;
    cin >> n >> m;
    if (m == 0) return void(cout << -1 << '\n');
    vector<Edge> edges;
    auto add = [&](int x, int y, int c) {
        if (x == n - 2 + n || x == n - 1) return;
        if (y == n - 2 + n || y == n - 1) return;
    ...
    edges.push_back({n - 2 + n + 1, n - 1 + 1, M});
    auto mwm = MWM(2 * n, edges);
    int val = M * n - mwm.maximum_weighted_matching(true);
    if (val >= n) return void(cout << -1 << '\n');
    auto mate = mwm.get_matching();
    vector<vector<int>> adj(n);
    for (int i = 1; i <= 2 * n; i++) {
        if (i == n || i == 2 * n - 1) continue;
        if (mate[i] < i) continue;
        if (mate[i] == i + n) continue;
        int u = i;
        int v = mate[i];
        --u, --v;
        if (u >= n) u -= n;
