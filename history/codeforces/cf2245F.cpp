...
void solve() {
    ...
    a[n] = -1;
    vector<int> off(n + 1);
    for (int i = 0; i < n; i++) off[i + 1] = off[i] + a[i] + 1;
    if (off[n] >= 2 * n) return void(cout << "0\n");

    vector f(n, vector<Z>(off[n]));
    vector g(n + 1, vector<Z>(n + 1));
    for (int r = 0; r <= n; r++) {
        g[r][r] = 1;
        if (a[r] != -1) f[r][off[r]] = 1;
        for (int l = r - 1; l >= 0; l--) {
            for (int i = l; i < r; i++) {
                if (a[i] == -1)
                    g[l][r] += g[l][i] * g[i + 1][r];
                else
                    g[l][r] += f[l][off[i] + a[i]] * g[i + 1][r];
            }
            g[l][r] *= comb.inv(r - l);
        }
        if (a[r] != -1) {
            for (int l = r - 1; l >= 0; l--) {
                for (int d = 1; d <= a[r]; d++) {
                    ...
                    for (int i = l; i < r; i++) {
                        if (a[i] == -1)
                            f[l][id] += g[l][i] * f[i + 1][id - 1];
                        else
                            f[l][id] += f[l][off[i] + a[i]] * f[i + 1][id - 1];
                    }
                    f[l][id] *= comb.inv(r - l);
                }
            }
