...
struct node {
    ...
    node *l, *r;
    node(T _v) : cnt(1), v(_v), lazy(0), p(mt_treap()), l(nullptr), r(nullptr) {}
};
...
template<typename T>
void splitv(node<T> *t, node<T> *&l, node<T> *&r, T v) { // splits into <= v and > v
    if (t == nullptr) {
        l = nullptr;
        r = nullptr;
    } else if (t->v < v) {
...
void pt(node<T> *r) {
    if (r == nullptr) return;
    pt(r->l);
    cout << r->v << "" << r->p << "" << r->cnt << endl;
    pt(r->r);
}

template<typename T>
void treap_union(vector<pair<T, T>> &store, node<T> *&t, node<T> *l, node<T> *r) {
    if (l == nullptr) {
        t = r;
    } else if (r == nullptr) {
        t = l;
        ...
        splitv(b, b, c, l->v + 1);
        if (b) {
            store.push_back({l->cnt, b->cnt});
            l->cnt += b->cnt;
            delete b;
        }
        treap_union(store, l->l, l->l, a);
        treap_union(store, l->r, l->r, c);
        t = l;
    }
}
...
void solve() {
    ...
    using tnode = node<int>;

    vector<pair<int, int>> store;

    y_combinator([&](auto self, int v) -> tnode * {
        d++;
        tnode *freq = nullptr;
        for (int w: ch[v]) {
            auto z = self(w);
            treap_union(store, freq, freq, z);
        }
        tnode *a, *l, *r, *b;
        splitv(freq, a, l, A[v]);
        splitv(l, l, r, A[v] + 1);
        splitv(r, r, b, A[v] + 2);
        int ncnt = cnt(l) + cnt(r) + 1;
        if (l) delete l;
        if (r) delete r;
        l = new node(A[v]);
        l->cnt = ncnt;
