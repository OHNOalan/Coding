...
void solve() {
    ...
    cin >> n >> m;
    vector<int> time(n);
    auto fill = [&](this auto &&fill, int l, int r, int start) -> void {
        if (l > r) return;
        int mid = (l + r) / 2;
        time[mid] = start;
        fill(l, mid - 1, start + 1);
        fill(mid + 1, r, start + 1);
    };
    fill(0, n - 1, 1);
    vector<int> l(n, -1), r(n, n);
    stack<int> s;
    for (int i = 0; i < n; i++) {
        while (!s.empty() && time[s.top()] > time[i]) {
            r[s.top()] = i;
            s.pop();
        }
        if (!s.empty()) {
    ...
    for (int sz = 1; sz < n + 1; sz++) {
        int out = n - sz;
        ans += v[sz] * comb.binom(out + m - 2, m - 3);
    }
    int mn = INT_MAX;
    for (int i = 0; i < n; i++) {
        mn = min(mn, time[i]);
        ans += comb.binom(n - i - 1 + m - 2, m - 2) * mn;
    }
    mn = INT_MAX;
    for (int i = n - 1; i >= 0; i--) {
        mn = min(mn, time[i]);
        ans += comb.binom(i + m - 2, m - 2) * mn;
    }
    cout << ans << '\n';
}
