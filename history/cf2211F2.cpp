...
Z SB(int K, int S) { return comb.binom(S + K - 1, S); }
...
void solve() {
    ...
    m--;
    Z tot = 0;
    auto dfs = [&](this auto &&dfs, int l, int r, int depth) -> void {
        if (l > r) return;
        int lp = l - 1;
        int rp = r + 1;
        int mid = (l + r) / 2;
        tot += SB(n + 1, m) * depth;
        if (lp != -1) tot -= SB(n + 1 - (mid - lp), m) * depth;
        if (rp != n) tot -= SB(n + 1 - (rp - mid), m) * depth;
        if (lp != -1 && rp != n) tot += SB(n + 1 - (rp - lp), m) * depth;
        dfs(l, mid - 1, depth + 1);
        dfs(mid + 1, r, depth + 1);
    };
    dfs(0, n - 1, 1);
