...
int solve() {
    ...
    for (int len = 2; len <= n; len++)
        for (int l = 1, r = len; r <= n; l++, r++) {
            ...
            for (int k = t - 1, m1 = s - 1, m2 = s; k > r; k--) {
                ...
                while (m1 >= l && abs(sum(l, m1) - sum(m1 + 1, r)) <= cur) mi = min(mi, get(dp, l, r, m1)), m1--;
                while (m2 < r && abs(sum(l, m2) - sum(m2 + 1, r)) <= cur) mi = min(mi, get(dp, l, r, m2)), m2++;
    ...
    for (int m = 1; m < n; m++) printf("%lld%c", get(dp, 1, n, m) < INF ? get(dp, 1, n, m) : -1LL, " \n"[m == n - 1]);
    return 0;
}
int main() {
    int T;
