...
struct LinkedList {
    ...
    void link(int u, int v) { nxt[u] = v, pre[v] = u; }
    void cut(int u, int v) { nxt[u] = u, pre[v] = v; }
} lst;
...
int main() {
    ...
    assert(mxc);
    auto flip = [&](int v) {
        for (size_t i = 0; i < e[v].size(); i++) lst.cut(get_eid(e[v][i], 1), get_eid(e[v][(i + 1) % e[v].size()], 0));
        reverse(e[v].begin(), e[v].end());
        for (size_t i = 0; i < e[v].size(); i++) lst.link(get_eid(e[v][i], 1), get_eid(e[v][(i + 1) % e[v].size()], 0));
    };
    ...
    auto adjust = [&](int v) {
        if (e[v].size() < 3)
            return 0;
        vector<int> cid;
        for (size_t i = 0; i < e[v].size(); i++) cid.push_back(bel[get_eid(e[v][i], 1)]);
        if (cid[0] == cid[1] && cid[0] == cid[2])
            return 0;
        if (cid[0] != cid[1] && cid[0] != cid[2] && cid[1] != cid[2]) {
            for (size_t i = 0; i < e[v].size(); i++)
                if (cid[i] != mxc)
                    relabel(get_eid(e[v][(i + 1) % e[v].size()], 0), get_eid(e[v][i], 1));
            flip(v);
            return 1;
        }
        int dif = -1;
