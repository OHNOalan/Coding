...
struct FastMod {
    ...
    ll inv(ll w) { return fp(w, Mod - 2); }
    void init(ll P) {
        Mod = P;
        k = 2 * (__lg(Mod - 1) + 1);
        r = ((__int128) 1 << k) / Mod;
        for (int i = (inv_fac[0] = 1); i <= N; ++i) inv_fac[i] = mul(inv_fac[i - 1], inv(i));
    }
} fm;
struct FFT {
    ...
    void dft(complex<db> a[], int n) {
        n /= 2;
        for (int i = 0; i < n; i++) a[i] = complex<db>(a[i * 2].real(), a[i * 2 + 1].real());
        fft(a, n, 1);
        for (int i = 0; i <= n / 2; ++i) {
            int j = (n - i) & (n - 1);
    ...
    void idft(complex<db> a[], int n) {
        ...
        fft(a, n, -1);
        for (int i = n - 1; i >= 0; --i) a[i * 2 + 1] = a[i].imag(), a[i * 2] = a[i].real();
    }
    void convolution(ll f[], ll g[], int n, ll Mod) {
        ...
        for (int i = 0; i < 2; i++) dft(p[i], n), dft(q[i], n);
        for (int i = 0; i < n; i++) {
            p[2][i] = p[0][i] * q[1][i] + p[1][i] * q[0][i];
            p[0][i] = p[0][i] * q[0][i], p[1][i] = p[1][i] * q[1][i];
        }
        for (int i = 0; i < 3; i++) idft(p[i], n);
        for (int i = 0; i < n; i++) {
            ll tmp[3];
            for (int j = 0; j < 3; j++) tmp[j] = llround(p[j][i].real()) % Mod;
    ...
    vector<ll> point_value_shift(const vector<ll> &f, int m, ll c, ll Mod) {
        ...
        for (int i = (fac[0] = 1); i <= n + m; i++) fac[i] = fm.mul(c - n - 1 + i, fac[i - 1]);
        inv_fac[n + m] = fm.inv(fac[n + m]);
        for (int i = n + m; i > 0; i--) inv_fac[i - 1] = fm.mul(c - n - 1 + i, inv_fac[i]);
        for (int i = 0; i < len; i++) b[i] = (i < n + m ? fm.mul(inv_fac[i + 1], fac[i]) : 0);
        convolution(a, b, len, Mod);
        vector<ll> res(m);
        for (int i = 0; i < m; i++) res[i] = fm.mul(a[i + n], fm.mul(fac[i + n + 1], inv_fac[i]));
        return res;
    }
} fft;
struct Solver {
    ll n, s;
    vector<ll> val, pri;
    int get(ll v) { return v < s ? v - 1 : val.size() - n / v; }
    void prepare(ll _n) {
        n = _n;
        val.clear();
        for (s = 1; s * s < n; s++) val.push_back(s);
        for (ll i = n / s; i >= 1; i--) val.push_back(n / i);
        pri.clear();
    ...
    vector<ll> init_dp(ll Mod) {
        ...
        for (int c = __lg(v) - 1, d = 1; c >= 0; c--) {
            ...
            for (int i = 0; i <= 2 * d; i++) pv[i] = fm.mul(pv[i], rv[i]);
            d *= 2;
            if (v >> c & 1) {
                for (int i = 0; i <= d; i++) pv[i] = fm.mul(pv[i], i * v + d + 1);
                pv.push_back(1);
                for (int j = 0; j <= d; j++) pv[d + 1] = fm.mul(pv[d + 1], (d + 1) * v + j + 1);
                d += 1;
            }
        }
        for (int i = 1; i <= v; i++) pv[i] = fm.mul(pv[i], pv[i - 1]);
        ...
        for (size_t i = (g[0] = 1), blk = sqrt(s); i < val.size(); i++) {
            if (val[i] <= fm.N)
                ...
                g[i] = g[i - 1];
                for (ll j = val[i - 1] + 1; j <= val[i]; j++) g[i] = fm.mul(g[i], j);
            } else {
                ll blk = min((ll) pv.size(), val[i] / v);
                g[i] = (blk > 0 ? pv[blk - 1] : 1);
                for (ll j = blk * v + 1; j <= val[i]; j++) g[i] = fm.mul(g[i], j);
            }
        }
        return g;
    }
    ...
    ll solve(ll n, ll Mod) {
        ...
        ll res = dp(Mod).back();
        for (auto p: pri)
            for (ll i = p; i <= n / p; i *= p) res = fm.mul(res, p);
        return res;
    }
} solver;
int main() {
