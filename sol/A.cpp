...
void div(const array<int, 4> &c, int &n, int &m) {
    ...
    for (int i = 0; i <= n; i++)
        for (int j = 0; j <= m; j++) cof[i][j] = cof[i + 1][j + 1], cof[i + 1][j + 1] = 0;
}
int w[MAXN], h[MAXN];
...
int main() {
    ...
    for (int r = 0; r < n; r++) {
        ...
        for (size_t i = 0; i + 1 < cx.size(); i++) {
            ...
            for (ssize_t j = cy.size() - 2; j >= 0; j--) {
                int a[2] = {0, 0}, b[2] = {0, 0};
                if (cx[i] >= w[r])
                    a[0] = (a[0] + w[r]) % Mod, a[1] = (a[1] - 1 + Mod) % Mod;
                if (cx[i] >= W - w[r])
                    a[0] = (a[0] + W - w[r]) % Mod;
                else
                    a[1] = (a[1] + 1) % Mod;
                if (cy[j] >= h[r])
                    b[0] = (b[0] + h[r]) % Mod, b[1] = (b[1] - 1 + Mod) % Mod;
                if (cy[j] >= H - h[r])
                    b[0] = (b[0] + H - h[r]) % Mod;
                else
                    b[1] = (b[1] + 1) % Mod;
    ...
    vector<int> inv(n + 2);
    for (int i = 1; i <= n + 1; i++) inv[i] = fp(i, -1);
    vector<vector<int>> pwx(cx.size(), vector<int>(n + 2));
    for (size_t i = 0; i < cx.size(); i++) {
        pwx[i][0] = 1;
        for (int j = 1; j <= n + 1; j++) pwx[i][j] = 1LL * pwx[i][j - 1] * cx[i] % Mod;
    }
    vector<vector<int>> pwy(cy.size(), vector<int>(n + 2));
    for (size_t i = 0; i < cy.size(); i++) {
        pwy[i][0] = 1;
        for (int j = 1; j <= n + 1; j++) pwy[i][j] = 1LL * pwy[i][j - 1] * cy[i] % Mod;
    }
    int res = 0;
    for (size_t i = 0; i + 1 < cx.size(); i++) {
        cof[0][0] = 1, zero = 0;
        ...
        for (int x = 0; x <= degx; x++)
            for (int y = 0; y <= degy; y++) cof[x][y] = 0;
    }
